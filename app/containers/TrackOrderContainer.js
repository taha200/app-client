...
class TrackOrderContainer extends React.Component {
    ...
    constructor(props) {
        ...
        console.log("current Dict ::::::::::::::: ", this.trackOrderDict)
        var userObj = null;
    }
    ...
    componentDidMount() {
        ...
        alert(JSON.stringify(this.trackOrderDict))
        this.driverTracking();
    ...
    driverTracking = () => {
        let param = {
      driver_id:this.trackOrderDict.data[0].driver_id
        }

        apiPost(
            "http://doofy.thinksolutionz.org/v1/driver_api/driverTrack",
            param,
            onSuccess => {
      ...
      this.setState({
          driver_latitude:onSuccess.driver_track.latitude,
          driver_longitude:onSuccess.driver_track.longitude
      })
      this.trackArrivalTime(this.state.driver_latitude,this.state.driver_longitude,this.trackOrderDict.data[0].user_latitude,this.trackOrderDict.data[0].user_longitude);
    ...
    trackArrivalTime = (sourceLat, sourcelong, destinationLat, destinationlong) => {
       ...
       fetch(arrival,{
           ...
           this.setState({
            timing:data.routes[0].legs[0].duration.text,
...
//                 alert(JSON.stringify(onSuccess.routes[0].legs[0].duration.text))
        ...
        navigator.geolocation.getCurrentPosition(
            position => {
                ...
                this.state.curr_latitude = parseFloat(position.coords.latitude),
                    this.state.curr_longitude = parseFloat(position.coords.longitude),
                    this.state.rest_latitude = parseFloat(this.trackOrderDict.resLat),
                    this.state.rest_longitude = parseFloat(this.trackOrderDict.resLat),
                    this.state.region = {
        ...
        return (
            <View style={styles.mainView}>
                <View style={{ height: metrics.screenHeight * 0.06, backgroundColor: EDColors.backgroundLight, justifyContent: 'center' }}>
                    <Text style={{ marginHorizontal: metrics.screenWidth * 0.036, fontSize: 20, fontFamily: ETFonts.bold }}>{this.trackOrderDict.title}</Text>
                </View>
                <View style={{ flexDirection: 'row' }}>
                    <View style={{ flex: 6, marginHorizontal: metrics.screenWidth * 0.036 }}>
                        <Text
                            style={{ fontFamily: ETFonts.bold, fontSize: 15, marginVertical: metrics.screenHeight * 0.021,color:"black" }}

                        >{`${this.state.distance}${` `}Far Away from you`}</Text>
                        {/* <DeliveryDetailComponent
                            style={{ marginBottom: metrics.screenHeight * 0.021 }}
                            source={asse.location_selected} label={this.currentOrderDict.address} /> */}
                        <DeliveryDetailComponent
                            style={{ marginBottom: metrics.screenHeight * 0.021 }}
                            source={asse.time}
                            label={"Arriving in:" + this.state.timing
                        ...
                        >{"Pay amount - $" + this.trackOrderDict.total}</Text> */}
                    </View>
                    <View style={{ flex: 2, alignItems: 'center', }}>
                       
                        <TouchableOpacity style={{ marginTop: - metrics.screenHeight * 0.025, marginLeft: metrics.screenWidth * 0.10, zIndex: 1000 }} onPress={() => this.connectToCall(this.trackOrderDict.driver.mobile_number)}>
                            <Image
                                style={{ width: 40, height: 40 }}
                                resizeMode='cover'
                                source={asse.call_order} />
                        </TouchableOpacity>
