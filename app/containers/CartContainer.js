...
import { saveCartData, getCartList, clearCartData } from "../utils/AsyncStorageHelper";
import {
  ADD_TO_CART,
  RESPONSE_SUCCESS,
  COUPON_ERROR,
    ...
    getCartList(
      success => {
        ...
        success.forEach((item)=>{
          var tox=parseFloat(item.tax)
          tax=tax+tox
          item.items.forEach((itema)=>{
               var cc = itema.price * itema.quantity
               tot=tot+cc
})
this.setState({
  total:tot,
  taxes:tax
      ...
      var per=this.state.total*(this.state.taxes/100)
      var por=this.state.total+per
    ...
    apiPost(
      ...
      onSuccess => {
          ...
          if(onSuccess.status === 0){
            this.setState({isLoading: false})
            alert("Delivery service is not available at this moment")
          }else{
          var checkoutData = {
            address_id: 0,
  ...
  render() {
    return (
      ...
      >
        ...
        {this.state.dataInCart != undefined && this.state.dataInCart.length > 0
          ? this.mainView()
          : this.state.dataInCart != undefined &&
            this.state.dataInCart.length <= 0
            ? this.emptyView()
            : null}
      </BaseContainer>
    );
  ...
  mainView() {
    return (
      <View style={{ flex: 1, paddingBottom: 5 }}>
        ...
        {this.state.dataInCart.length > 0 && this.state.dataInCart.map((itema,ind)=>{
          return(
            <View>
              {itema.items.map((item,index)=>{
    return (
      <CartItem
        ...
        onPlusClick={value => {
          if (value > 0) {
            var arr = this.state.dataInCart
            var val = this.state.total
          
            var inT=parseInt(arr[ind].items[index].price)
            arr[ind].items[index].quantity=value;
          
            val=val+inT
            var per=val*(this.state.taxes/100)
            var por=val+per
            ...
            saveCartData(arr, success => { }, fail => { });

          }
        }}
        onMoreThanFive={(value,ol) => {
          if (value > 0) {
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                if (this.props.userID != undefined && this.props.userID != "") {
                  ...
                  //   this.checkOrderAPI()
                  // }else{
                    AsyncStorage.getItem("promoCode").then(
                      res => {
                      ...
                      var checkoutData = {
                        address_id: 0,
                        items:this.state.dataInCart,
                        user_id: this.props.userID,
                        token: this.props.token,
                        total: this.state.total,
                       totals:this.state.totals,
                       coupon_name:this.state.coup
                      };
          ...
          <View style={style.optionContainer}>
            <Text
              ...
              onPress={() => {
               ...
               var arr=this.state.dataInCart
              //  var price =parseInt(arr[this.deleteIndex].items[this.deleteItemIndex].price);
              //  var quantity=parseInt(arr[this.deleteIndex].items[this.deleteItemIndex].quantity);
              //   var abc=price*quantity
              //   var bcd=this.state.total-abc
              //   var per=bcd*(this.state.taxes/100)
              //   var por=this.state.total+per
               arr[this.deleteIndex].items.splice(this.deleteItemIndex,1)
               if(arr[this.deleteIndex].items.length===0){
                 var a=arr[this.deleteIndex].tax
                arr.splice(this.deleteIndex,1)

                this.setState({
                 taxes:this.state.taxes-a
                })
               }
